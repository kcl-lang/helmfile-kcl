...
: "${HELM_PLUGIN_DIR:="$HELM_HOME/plugins/helmfile-kcl"}"
...
getDownloadURL() {
  ...
  if [ "$SCRIPT_MODE" = "install" ] && [ -n "$version" ]; then
    DOWNLOAD_URL="https://github.com/$PROJECT_GH/releases/download/$version/helmfile-kcl-$OS-$ARCH.tgz"
  else
    DOWNLOAD_URL="https://github.com/$PROJECT_GH/releases/latest/download/helmfile-kcl-$OS-$ARCH.tgz"
  fi
}
...
rmTempDir() {
  if [ -d "${HELM_TMP:-/tmp/helmfile-kcl-tmp}" ]; then
    rm -rf "${HELM_TMP:-/tmp/helmfile-kcl-tmp}"
  fi
}
...
installFile() {
  ...
  HELM_TMP_BIN="$HELM_TMP/kcl/bin/kcl"
  if [ "${OS}" = "windows" ]; then
...
exit_trap() {
  ...
  if [ "$result" != "0" ]; then
    ...
    printf '\tFor support, go to https://github.com/KusionStack/helmfile-kcl.\n'
  fi
  exit $result
}
