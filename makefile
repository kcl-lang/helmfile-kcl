...
.PHONY: install
install: build
	mkdir -p $(HELM_HOME)/plugins/helmfile-kcl/bin
	cp -f bin/kcl $(HELM_HOME)/plugins/helmfile-kcl/bin
	cp -f plugin.yaml $(HELM_HOME)/plugins/helmfile-kcl/

.PHONY: install/helm3
install/helm3: build
	mkdir -p $(HELM_3_PLUGINS)/helmfile-kcl/bin
	cp -f bin/kcl $(HELM_3_PLUGINS)/helmfile-kcl/bin
	cp -f plugin.yaml $(HELM_3_PLUGINS)/helmfile-kcl/
...
docker-run-release: export pkg=/go/src/github.com/KusionStack/helmfile-kcl
docker-run-release:
	git checkout main
	git push
	docker run -it --rm -e GITHUB_TOKEN -v $(shell pwd):$(pkg) -w $(pkg) golang:1.19 make bootstrap release
